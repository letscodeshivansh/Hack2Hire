...
app.get("/profile", async (req, res) => {
  ...
  try {
    const user = await User.findById(req.session.loggedInUsername);
    if (!user) {
...
app.post("/profile", async (req, res) => {
  ...
  try {
    const updates = {
      ...
      education: req.body.education.split(","),
      projects: req.body.projects.split(","),
      skills: req.body.skills.split(","),
    };

    await User.findByIdAndUpdate(req.session.loggedInUsername, updates);
...
// Login route for testing (placeholder)
app.get("/login", (req, res) => {
